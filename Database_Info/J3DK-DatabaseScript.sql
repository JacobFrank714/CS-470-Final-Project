...
BEGIN
set createTime = NOW();
INSERT INTO `Albums` (Album_Name,Artistm,Num_songs,Album_Cover,Discription,Release_Date)
VALUES(Name, Artist, Num_Songs, Album_Cover, Description,creatTime);
END;$$
...
BEGIN
insert into `j3dk`.`Playlist_Songs` (Song_Name,Playlist_Name)
values (song,playlist);

select Song_Len from `j3dk`.`Songs` as song_len where Song_Name = song and Artist = artist;

update `j3dk`.`Playlist`
set Playlist_Len = Playlist_Len + song_len.value()
